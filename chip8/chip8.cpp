...
int main(int argc, char* argv[])
{
    ...
    while (!close) {
...

        {
            ...
            // if (byte & (1 << j)) {
            //     if (render_scale > 1) {
            //         SDL_Rect rect { (x_origin + (7 - j)) * render_scale, (y_origin + row) * render_scale, render_scale, render_scale };
            //         SDL_RenderDrawRect(&renderer, &rect);
            //         SDL_RenderFillRect(&renderer, &rect);
            //     } else {
            //         SDL_RenderDrawPoint(&renderer, (x_origin + (7 - j)) * render_scale, (y_origin + row) * render_scale);
            //     }
            // }
        }
